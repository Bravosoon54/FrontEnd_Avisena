...
function createInvRow(inv) {
  ...
  if (inv.categoria) {
    ...
    else if (typeof inv.categoria === 'object') categoriaText = inv.categoria.nombre || inv.categoria.name || '';
    else categoriaText = String(inv.categoria);
  }
  // Si aún no tenemos texto, intentar resolver por id usando los mapas cargados
  if (!categoriaText) {
    const possibleIds = [inv.id_categoria, inv.id_cat, inv.categoria, inv.category_id, inv.category];
  ...
  if (inv.finca) {
    ...
    else if (typeof inv.finca === 'object') fincaText = inv.finca.nombre || inv.finca.name || '';
    else fincaText = String(inv.finca);
  }
  // Resolver por id desde el mapa si es necesario
  if (!fincaText) {
    const possibleIds = [inv.id_finca, inv.idFarm, inv.finca, inv.farm_id, inv.farm];
    for (const pid of possibleIds) {
      if (pid !== undefined && pid !== null) {
  ...
  return `
    <tr data-id="${invId}">
      <td class="px-0 fs-5">${nombre}</td>
      <td class="px-0 fs-5">${cantidad}</td>
      <td class="px-0 fs-5">${unidad_medida}</td>
      <td class="px-0 fs-5">${descripcion}</td>
      <td class="px-0 fs-5">${categoriaText}</td>
      <td class="px-0 fs-5">${fincaText}</td>
      <td class="px-0">
...
async function handleCreateSubmit(e) {
  ...
  if (!nombre || !cantidad || !unidad_medida || !descripcion || !id_categoria || !id_finca) {
    Swal.fire({
  ...
  try {
    await inventoryService.createInventory({ nombre, cantidad, unidad_medida, descripcion, id_categoria, id_finca});
...
async function handleUpdateSubmit(e) {
  ...
  try {
    await inventoryService.updateInventory(id, { nombre, cantidad, unidad_medida, descripcion, id_categoria, id_finca });
    
    // Cerrar el modal
    const modal = bootstrap.Modal.getInstance(document.getElementById('edit-inv-modal'));
    if (modal) modal.hide();
...
async function handleTableClick(e) {
  ...
  if (editBtn) {
    ...
    try {
      ...
      const catId = inv.id_categoria || (inv.categoria && (inv.categoria.id_categoria || inv.categoria.id || inv.categoria.id_cat)) || inv.category || '';
      if (catInput) catInput.value = (catId !== undefined && catId !== null) ? String(catId) : '';

      // Extraer id de finca/land robustamente (igual que categoría)
      const finId = inv.id_finca || (inv.finca && (inv.finca.id_finca || inv.finca.id || inv.finca.idLand || inv.finca._id)) || inv.farm || '';
      if (finInput) finInput.value = (finId !== undefined && finId !== null) ? String(finId) : '';
...
async function cargarCategorias() {
  try {
    ...
    if (selectCrear) {
      ...
      const placeholderOpt = selectCrear.querySelector('option[value=""]') ? selectCrear.querySelector('option[value=""]').outerHTML : '<option value="">Seleccionar categoría</option>';
      selectCrear.innerHTML = placeholderOpt;
      const opts = categorias.map(tipoOrig => {
        const { id, nombre } = normalizeTipo(tipoOrig);
        // guardar en mapa
        if (id !== null && id !== undefined) categoryMap.set(String(id), nombre);
    ...
    if (selectEditar) {
      ...
      const placeholderOptE = selectEditar.querySelector('option[value=""]') ? selectEditar.querySelector('option[value=""]').outerHTML : '<option value="">Seleccionar categoría</option>';
...
async function cargarFincas() {
  try {
    ...
    const fincasActivas = (Array.isArray(fincas) ? fincas : []).filter(f => {
      console.log(`DEBUG - finca: ${f.nombre}, estado: ${f.estado}, tipo: ${typeof f.estado}`);
    ...
    if (selectCrear) {
      // limpiar opciones previas (conservar placeholder si existe)
      const placeholderOptF = selectCrear.querySelector('option[value=""]') ? selectCrear.querySelector('option[value=""]').outerHTML : '<option value="">Seleccionar finca</option>';
      selectCrear.innerHTML = placeholderOptF;
      const opts = (Array.isArray(fincasActivas) ? fincasActivas : []).map(fOrig => {
        const { id, nombre } = normalizeFinca(fOrig);
        if (id !== null && id !== undefined) fincaMap.set(String(id), nombre);
        if (id !== null && id !== undefined) return `<option value="${id}">${nombre}</option>`;
        return '';
      }).join('');
      selectCrear.insertAdjacentHTML('beforeend', opts);
    }
    ...
    if (filterSelect) {
      ...
      const placeholder = filterSelect.querySelector('option[value=""]') ? filterSelect.querySelector('option[value=""]').outerHTML : '<option value="" disabled selected>Seleccione la finca</option>';
      filterSelect.innerHTML = placeholder;
      const optsF = (Array.isArray(fincasActivas) ? fincasActivas : []).map(fOrig => {
        const { id, nombre } = normalizeFinca(fOrig);
        if (id !== null && id !== undefined) {
    ...
    if (selectEditar) {
      ...
      // limpiar opciones previas (conservar placeholder si existe)
      const placeholderOptFE = selectEditar.querySelector('option[value=""]') ? selectEditar.querySelector('option[value=""]').outerHTML : '<option value="">Seleccionar finca</option>';
...
async function loadInventoryByLand(landId) {
  ...
  if (!tableBody) return;
  tableBody.innerHTML = '<tr><td colspan="6" class="text-center">Cargando Inventario...</td></tr>';
  try {
    ...
    if (inventory && inventory.length > 0) {
      ...
      tableBody.innerHTML = '<tr><td colspan="6" class="text-center">No se encontraron Items en el inventario para esta finca.</td></tr>';
    }
  } catch (err) {
...
async function init() {
  ...
  tableBody.innerHTML = '<tr><td colspan="6" class="text-center">Cargando Inventario...</td></tr>'; // ✅ CORRECCIÓN: colspan="6"
  ...
  if (filterSelect) {
    ...
    else tableBody.innerHTML = '<tr><td colspan="6" class="text-center">Selecciona una finca para ver su inventario.</td></tr>';
    ...
    try {
      ...
      if (inventory && inventory.length > 0) {
        ...
        tableBody.innerHTML = '<tr><td colspan="6" class="text-center">No se encontraron Items en el inventario.</td></tr>';
      }
    } catch (error) {
